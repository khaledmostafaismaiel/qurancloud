...
$(document).ready(function () {
        ...
        if(with_side_nav == 1){
            ...
            $.ajax({
                ...
                success:function (response) {
                    ...
                    // loadMoreShowChatMessages(showChatMessagesPageNumber,chat_id);
                }
            });
        }else{
            $.ajax({
    ...
    $(document).on('click','.edit-track',function () {
        ...
        $(this).closest('div#track-' + track_id)
            .find('div.card-body input.card-text')
            .attr('readonly',false)
            .removeClass('text-white bg-dark border-0')
            .addClass('text-dark bg-white border');
        $(this).closest('div#track-' + track_id)
            .find('div.card-body button.submit_edit_track')
            .removeClass('d-none');

    });

    $(document).on('click','button.submit_edit_track',updateTrackFromSubmit);
    // $(document).keypress('.edit_track',updateTrackFromEnter);
    ...
    $(document).on('click','button.submit_edit_comment',updateTrackCommentFromSubmit);
    // $(document).keypress('input[name=edit_comment]',updateTrackCommentFromEnter);
        ...
        $.ajax({
            ...
            success:function (response) {
                $(event.target).attr('readonly',true)
                    .addClass('text-white bg-dark border-0')
                    .removeClass('text-dark bg-white border');
                $(event.target).closest('div.card-body')
                    .find('button.submit_edit_track')
                    .addClass('d-none');
                $.toaster({ priority : response.priority, title : response.title, message : response.message });
            },
            error:function () {
                $.toaster({ priority : "danger", title : "Error", message : "Try again,sorry!" });
            }
        });
...
function updateTrackFromSubmit(event) {
    ...
    var newCaption = $(this).closest('div.card-body')
        .find('input.card-text').val();
    $.ajaxSetup({
        headers:{
            'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')
        }
...
function followAction(event) {
    ...
    $.ajax({
        ...
        success:function (response) {
            $(event.target).attr('data-friends_id',response.friends_id);
...
function updateTrackCommentFromEnter(event) {
    if(event.which == 13){
        ...
        $.ajax({
            ...
            success:function (response) {
                $(event.target).closest('table').find('tr input[name=edit_comment]')
...
function allowUpdateTrackComment(){
    $(this).closest('table').find('tr input[name=edit_comment]').removeClass('bg-dark')
...
function updateTrackCommentFromSubmit(event) {
    ...
    $.ajax({
        ...
        success:function (response) {
            $(event.target).closest('table').find('tr input[name=edit_comment]').removeClass('bg-white')
                .removeClass('border')
                .removeClass('text-dark')
                .addClass('bg-dark')
                .addClass('border-0')
...
function loveComment(event){
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.toaster({ priority : response.priority, title : response.title, message : response.message,has_progress:false,has_icon:true });
        },
        error:function () {
            $.toaster({ priority : "danger", title : "Error", message : "Try again,sorry!" });
        }
...
function loadMoreTrackCommentLoves(pageNumber="",comment_id=""){
    if(! $('#spinning-loader-btn').length) {
...
function playMethod(event) {
            ...
            if(! audioPlayer){
                ...
                //time update
                audioPlayer.addEventListener('timeupdate',timeUpdateMethod);
...
function use_swal(message,status="success") {
    swal({
        position: 'top-end',
        icon: status,
        title: message,
